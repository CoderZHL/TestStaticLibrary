...
public protocol EngineType {
    ...
    /// - Returns: 用于取消执行的句柄对象
    func call<Request: RequestType, Parser: ParserType>(_ request: Request, with parameter: Request.Parameter, parsedBy parser: Parser, completionHandler: @escaping (Result<Parser.Content>) -> Void) -> Cancelable
}
...
extension DefaultEngine: EngineType {
    public func call<Request: RequestType, Parser: ParserType>(_ request: Request, with parameter: Request.Parameter, parsedBy parser: Parser, completionHandler: @escaping (Result<Parser.Content>) -> Void) -> Cancelable {
        ...
        let cancelable = self.executor.execute(urlRequest: urlRequest, multipartFormData: multipartFormData) { data, response, error in
            ...
            DispatchQueue.global(qos: .utility).async {
                ...
                let result: Result<Parser.Content>
                do {
